...
sudo yum -y install oracle-database-preinstall-21c unzip &>/dev/null
...
EOT
    ;;
  8)
    sudo dnf -y install oracle-epel-release-el8 &>/dev/null
    sudo dnf -y --enablerepo=ol8_developer_EPEL install rlwrap &>/dev/null
    sudo tee -a /home/oracle/.bashrc <<EOT >/dev/null
alias sqlplus='rlwrap sqlplus'
EOT
    ;;
...
sudo su - oracle -c "
    unzip -o '$file' -d '$ORACLE_HOME' |& \
    awk -v total='$total_files' '
        BEGIN { count = 0 }
        /inflating:|extracting:/ {
            count++
            percent = int(count * 100 / total)
            printf \"\\r\\033[K  [%3d%%] Extracting: %d/%d files\", percent, count, total
            fflush()
        }
        END {
            if (count > 0) {
                printf \"\\r\\033[K  [100%%] Extracted: %d/%d files\\n\", count, count
            }
        }
    '
"
...
if [[ ${ORACLE_SAMPLESCHEMA^^} == TRUE ]]; then
  ...
  progress "Installing sample schemas"
  sample_dir=$(mktemp -d)
  readonly sample_dir
  chmod 777 "$sample_dir"
  cp "$script_dir"/install_sample.sh "$sample_dir"/install_sample.sh
  sudo su - oracle -c "$sample_dir/install_sample.sh $ORACLE_PASSWORD localhost/$ORACLE_PDB" &>/dev/null
  sudo su - oracle -c "rm -rf $sample_dir/*"
  rmdir "$sample_dir"
  success "Sample schemas installed"
fi

echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}✓ Oracle Database 21c installation completed!${NC}"
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
